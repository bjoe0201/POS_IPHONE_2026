...
| 階段 | 內容 | 對應任務 | 產出 |
|------|------|---------|------|
| **M0** | 專案骨架：`project.yml`、App 進入點、GRDB、Info.plist、iOS15 target | #1 | 可 `xcodegen generate` 開啟（待你裝 Xcode 驗證建置） |
| **M1** | 資料層：6 模型 + DB seeding/migration + repositories + SettingsStore | #2 | 資料層完成、可被上層注入 |
| **M2** | 登入 + 六分頁導航殼 + 分頁開關 | #3 | App 可登入、切分頁 |
| **M3** | 記帳點餐（核心）：選桌、分類、菜單卡、長按連加減、結帳、補登模式 | #4 | 日常營運可用 |
| **M4** | 菜單管理 + 桌號設定 | #5 | 主檔維護可用 |
| **M5** | 訂位：月曆、時段格線、訂位 CRUD | #6 | 訂位可用 |
| **M6** | 報表 + 圓餅圖 + CSV + PDF + AirPrint | #7 | 報表與列印可用 |
| **M7** | 設定頁 + ZIP 備份匯出/匯入 + 資料庫初始化 | #8 | 全功能對等 |
