...
`.xcodeproj` 由 XcodeGen 產生且不入版控，故 CI 需特別處理：

- **`ci_scripts/ci_post_clone.sh`**（Xcode Cloud 標準鉤子）：clone 後 `brew install xcodegen` → `xcodegen generate` → 把版控副本 `ci_scripts/Package.resolved` 複製到 `…/project.xcworkspace/xcshareddata/swiftpm/Package.resolved`。
- **為何用複製而非解析**：Xcode Cloud 關閉自動套件解析、且 post-clone 沙箱不允許網路解析（`xcodebuild -resolvePackageDependencies` 會 exit 74）；而 `xcodegen generate` 又會清掉 workspace，故只能在 generate 之後複製。
- **共用 scheme**：`project.yml` 定義 `schemes.POS`，產生 `xcshareddata/xcschemes/POS.xcscheme`，CI 才找得到。
- ⚠️ **同步維護**：每次變更 `project.yml` 的套件版本後，需在本機重新解析並更新版控的 `ci_scripts/Package.resolved`：
  ```bash
  xcodegen generate
  xcodebuild -resolvePackageDependencies -project POS_IPHONE_2026.xcodeproj -scheme POS
  cp POS_IPHONE_2026.xcodeproj/project.xcworkspace/xcshareddata/swiftpm/Package.resolved ci_scripts/Package.resolved
  ```
  （`.gitignore` 全域排除 `Package.resolved`，但以 `!ci_scripts/Package.resolved` 例外保留此副本。）
